...
默认安装到 Codex。`scripts/install.py` 会安装 `skills/*` 下的所有子 skill，目前包括主路由 `engineering-everything` 和 pilot 子 skill `engineering-project-inheritance`：
...
| `skills/engineering-project-inheritance/SKILL.md` | 接手已有项目 pilot 子 skill，处理老项目盘点和重构前置判断。 |
