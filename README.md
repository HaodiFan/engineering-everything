...
这个项目公开名称统一为 `engineering-everything`。新会话推荐入口是 `$using-engineering-everything` bootloader；`$engineering-everything` 保留为直达主路由。
...
如果你已经知道要直接进入主路由，也可以使用 `$engineering-everything`。
...
默认安装到 Codex。`scripts/install.py` 会安装 `skills/*` 下的所有子 skill，包括 bootloader、主路由和各场景子 skill：
...
| 脚本 | 作用 |
|---|---|
| `scripts/install.py` | 安装、升级、list、relink 或卸载 Codex / Agents skill 目录，避免复制路径漂移。 |
| `scripts/sync_references.py` | 检查 root reference 和子 skill runtime copy，防止 drift、orphan 和未声明 root-only。 |
| `scripts/eval_scenarios.py` | 校验 `evals/scenarios/*.md` 行为评估场景的 route、输出字段和 reference。 |
| `scripts/skill_doctor.py` | 发布前检查包结构、版本一致性、README、route 契约、reference 分发、eval 场景和旧触发名残留。 |
...
│   └── designs/
│       └── 2026-06-28-agentos-bootloader-refactor.md
├── evals/
│   └── scenarios/
├── skills/
│   ├── using-engineering-everything/
...
│       ├── SKILL.md
│       └── references/
├── references/                      # canonical reference source
│   ├── output-contracts.md
│   ├── route-contract.md
│   ├── codex-tools.md
...
| `skills/using-engineering-everything/SKILL.md` | bootloader，负责新会话、resume 和任务切换时进入主路由。 |
...
| `references/output-contracts.md` | 规划、执行、review 和行为 eval 的输出契约事实源。 |
| `data/reference_distribution.yaml` | root reference 到子 skill reference 副本的分发事实源。 |
| `evals/scenarios/*.md` | bootloader 和 route 行为的最小评估场景。 |
...
推荐触发名是 `$using-engineering-everything`，它会先做 bootloader 判断；`$engineering-everything` 是直达主路由。安装目录使用同名 skill 目录，repo 名仍是 `engineering-everything`。
