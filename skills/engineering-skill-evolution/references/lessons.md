...
> **本文件是 skill 自身的「纠偏沉淀」运行态日志**。每次用户纠正 agent 的方案，agent 必须先询问是否创建 GitHub lesson issue；只有 issue 去重并确认要进入 runtime skill 后，才追加到本文件。**只追加，不删除**。被推翻或废弃的 lesson 标 `Status: deprecated` 并写明替代项。
>
...
```
GitHub lesson issue          新纠偏候选，先审阅和去重
   ↓ 确认进入 runtime skill
L1 Lessons（本文件）         单条纠偏，立即生效
   ↓ ≥ 2 次命中或用户确认
...
用户回复包含以下信号时，**agent 必须**先承认具体偏差，并在该轮回复末尾追加一段「📌 是否创建 lesson issue」的提议。用户确认后只创建 GitHub issue，不改 `SKILL.md`、reference、scripts 或本文件；只有 issue 确认要通过 PR/commit 进入 runtime skill 时，才追加 L-NNNN：

- 否定/纠偏/不满意：「不对」「不是」「应该是」「这个方案有问题」「这里不行」「错了」「这部分好乱」「你没有参考」「方案不行」
