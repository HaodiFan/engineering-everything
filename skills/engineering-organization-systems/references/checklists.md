...
> 让 skill 自我进化：用户每次不满意或纠偏都先被提示是否创建 GitHub lesson issue；创建 issue 只做 intake，不改 runtime 文件。issue 确认进入 runtime 后，才通过 PR/commit 写入 `references/lessons.md`（L1）。多条同主题 lesson 合并到 `references/patterns-skill.md`（L2）；pattern 验证够多后开 PR 升级到对应 reference（L3）。详见 SKILL.md「Knowhow 沉淀规则」。
...
- [ ] 用户回复包含捕获信号（不对 / 不是 / 这部分好乱 / 你没有参考 / 应该是 / 错了 / 我之前是 / 不要 X 要 Y / 这一步不该现在做）→ 先承认具体偏差，再在该轮回复末尾追加「📌 是否创建 lesson issue」提议。
- [ ] 信号弱 / 单纯澄清 / 闲聊 → 不硬捕获。

### 会话结束时

- [ ] 触发了捕获信号且用户确认 → 先创建 GitHub lesson issue，不改 `SKILL.md` / reference / scripts / 安装副本；只有 issue 确认进入 runtime skill 后，才通过 PR/commit 按 `lessons.md` 模板追加 L-NNNN（连续编号）。
...
- 用户给出明确纠偏或不满意后，agent 不提示创建 GitHub lesson issue，下次同类问题再犯同样错。
- 用户只确认创建 lesson issue，agent 却顺手修改 `SKILL.md`、reference、scripts 或安装副本。
